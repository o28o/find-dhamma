...
nice -19 egrep -A1 -Eir "dn3[34].*(Dv|Tis|Tay|Tī|Cattā|Cata|Pañc|cha|Satta|Aṭṭh|Nav|das).{0,9}${defpattern}|Kata.*${defpattern}.{0,4}\\?|Kiñ.*${defpattern}.{0,9} vadeth|${defpattern}.*adhivacan|${defpattern}.*vucati|${defpattern}.{0,4}, ${defpattern}.*vucca" $suttapath/$pali_or_lang --exclude-dir={$sutta,$abhi,$vin,xplayground,name,site} --exclude-dir={ab,bv,cnd,cp,ja,kp,mil,mnd,ne,pe,ps,pv,tha-ap,thi-ap,vv} 
}
#\bKatha.*${defpattern}|
elif [[ "$@" == *"-all"* ]]; then
function grepbasefile {
